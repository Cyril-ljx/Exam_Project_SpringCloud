...
public class UserController {
    ...
    public Map<String, Object> updateUser(@RequestParam("id") Integer id, @RequestParam("username") String username, @RequestParam("password") String password, @RequestParam("email") String email, @RequestParam("name") String name, @RequestParam("address") String address, @RequestParam("phone") String phone, @RequestParam("user_type") String user_type) {
        Map<String, Object> dataMap = new HashMap<>();
        Boolean isSuccess = false;
        if (id != null) {
            //根据用户对象的id 查询用户的密码，防止密码丢失
            ...
            if (user1 != null) {
                //user.setPassword(user1.getPassword());
                //更新对象
                int update = userService.updateByPrimaryKey(id, username, password,  email, name, address, phone, user_type);
                if (update > 0) {
                    isSuccess = true;
                    dataMap.put("success", isSuccess);
                    return dataMap;
    ...
    public Map<String, Object> updatePwd(@RequestBody JSONObject json,Integer id) {
        Map<String, Object> dataMap = new HashMap<>();
        boolean isSuccess = false;
